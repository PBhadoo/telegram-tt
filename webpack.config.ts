...
const DEFAULT_APP_TITLE = `Telegram Web A (MOD)${APP_ENV !== 'production' ? ' Beta' : ''}`;
...
const CSP = `
  default-src 'self';
  connect-src 'self' wss://*.web.telegram.org wss://*.workers.dev blob: http: https: wss: ${APP_ENV === 'development' ? 'ipc:' : ''};
